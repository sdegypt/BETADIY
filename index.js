...
io.on('connection', (socket) => {
  ...
  socket.on('chat:send', async (data) => {
    console.log('Received chat:send:', data);
    try {
      const { from, to, content } = data;
      if (!from || !to || !content) return;
      // تحقق من صحة المستخدمين
      const fromUser = await UsersModels.getUserById(from);
      const toUser = await UsersModels.getUserById(to);
      if (!fromUser || !toUser) return;
      // حفظ الرسالة
      const saved = await chatModel.saveMessage(from, to, content);
      if (!saved) return;
      // إرسال الرسالة للطرفين فقط
      [from, to].forEach(uid => {
        const sid = userSocketMap.get(uid);
        if (sid) io.to(sid).emit('chat:receive', saved);
      });
    } catch (err) {
